...
export const OrderList: React.FC<OrderListProps> = ({ orders, onEdit, onDelete, onRestore, onSync, isSyncing, onStatusChange, onBatchDelete, onBatchStatusChange, onImport, onBatchLogisticsUpdate, initialFilter = 'All', isTrash = false, warningRules }) => {
  ...
  const handleBatchExportAction = () => {
      ...
      const headers = ["订单ID", "客户单号", "商品名称", "数量", "金额(USD)", "总价(USD)", "状态", "详细物流状态", "采购日期", "平台", "平台订单号", "收货地址", "TIKTOK平台单号", "入库物流单号", "备注"];
  ...
  // Fix: Added missing handleDelete handler to address 'Cannot find name' error
  const handleDelete = (id: string, e: React.MouseEvent) => {
    e.stopPropagation();
    onDelete(id);
  };

  // Fix: Added missing handleRestore handler to address 'Cannot find name' error
    ...
    <div className="space-y-6 animate-fade-in pb-20 relative">
          ...
          <div className="p-4 flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
            <div className="flex items-center gap-2 w-full md:w-auto flex-1">
                <div className={`relative w-full md:max-w-lg group transition-all duration-300 ${isSmartSearch ? 'scale-[1.01]' : ''}`}>
                    ...
                    {isAiThinking && <div className="absolute right-3 top-1/2 transform -translate-y-1/2"><Loader2 size={16} className="animate-spin text-indigo-500" /></div>}
                </div>
                {!isTrash && (
                    <button onClick={() => setShowFilters(!showFilters)} className={`p-2.5 rounded-lg border transition-all flex items-center gap-2 shrink-0 ${showFilters || activeFilterCount > 0 ? 'bg-indigo-50 dark:bg-indigo-900/30 border-indigo-200 dark:border-indigo-800 text-indigo-600 dark:text-indigo-400' : 'bg-white dark:bg-slate-900 border-slate-200 dark:border-slate-700 text-slate-500 dark:text-slate-400 hover:bg-slate-50 dark:hover:bg-slate-800'}`}>
                        <Filter size={18} /> {activeFilterCount > 0 && <span className="flex h-4 w-4 items-center justify-center rounded-full bg-indigo-600 text-[10px] font-bold text-white">{activeFilterCount}</span>}
                    </button>
                )}
            </div>
                ...
                {isTrash ? <span className="text-xs text-slate-500 dark:text-slate-400 font-medium px-2">{filteredOrders.length} 个订单在回收站</span> : (
                    <>
                        ...
                        <div className="flex items-center bg-slate-100 dark:bg-slate-800 p-1 rounded-lg border border-slate-200 dark:border-slate-700">
                            <button onClick={() => setViewMode('table')} className={`p-2 rounded-md transition-all flex items-center gap-2 text-xs font-medium ${viewMode === 'table' ? 'bg-white dark:bg-slate-700 shadow-sm text-indigo-600 dark:text-indigo-400' : 'text-slate-500 dark:text-slate-400 hover:text-slate-700 dark:hover:text-slate-200'}`}><List size={16} /></button>
                            <button onClick={() => setViewMode('grid')} className={`p-2 rounded-md transition-all flex items-center gap-2 text-xs font-medium ${viewMode === 'grid' ? 'bg-white dark:bg-slate-700 shadow-sm text-indigo-600 dark:text-indigo-400' : 'text-slate-500 dark:text-slate-400 hover:text-slate-700 dark:hover:text-slate-200'}`}><Grid size={16} /></button>
                            <button onClick={() => setViewMode('board')} className={`p-2 rounded-md transition-all flex items-center gap-2 text-xs font-medium ${viewMode === 'board' ? 'bg-white dark:bg-slate-700 shadow-sm text-indigo-600 dark:text-indigo-400' : 'text-slate-500 dark:text-slate-400 hover:text-slate-700 dark:hover:text-slate-200'}`}><Columns size={16} /></button>
                        </div>
                    </>
                )}
            </div>
          ...
          {showFilters && !isTrash && (
            <div className="bg-slate-50 dark:bg-slate-800/50 border-t border-slate-200 dark:border-slate-800 p-4 animate-fade-in">
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 items-end">
                     <div><label className="block text-xs font-bold text-slate-500 dark:text-slate-400 mb-1.5">采购日期范围</label><div className="flex gap-2"><input type="date" value={dateRange.start} onChange={(e) => setDateRange({...dateRange, start: e.target.value})} className="w-full text-xs px-2 py-1.5 rounded border border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-700 dark:text-white" /><span className="text-slate-400 self-center">-</span><input type="date" value={dateRange.end} onChange={(e) => setDateRange({...dateRange, end: e.target.value})} className="w-full text-xs px-2 py-1.5 rounded border border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-700 dark:text-white" /></div></div>
                     <div><label className="block text-xs font-bold text-slate-500 dark:text-slate-400 mb-1.5">采购平台</label><select value={platformFilter} onChange={(e) => setPlatformFilter(e.target.value)} className="w-full text-xs px-2 py-1.5 rounded border border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-700 dark:text-white h-[30px]"><option value="All">所有平台</option>{platforms.map(p => <option key={p} value={p}>{p}</option>)}</select></div>
                     <div className="pb-1"><label className="flex items-center gap-2 cursor-pointer select-none"><input type="checkbox" checked={hasNotesFilter} onChange={(e) => setHasNotesFilter(e.target.checked)} className="w-4 h-4 rounded border-slate-300 text-indigo-600 focus:ring-indigo-500" /><span className="text-xs font-medium text-slate-700 dark:text-slate-300">仅显示有备注的订单</span></label></div>
                     <div className="pb-1"><button onClick={clearFilters} className="w-full flex items-center justify-center gap-2 px-3 py-1.5 text-sm text-slate-500 dark:text-slate-400 hover:text-red-600 hover:bg-red-50 dark:hover:bg-red-900/20 rounded-md transition-colors"><XCircle size={14} />清空筛选条件</button></div>
                </div>
            </div>
          )}
      </div>
      ...
      {!isTrash && viewMode !== 'board' && (
        <div className="flex overflow-x-auto pb-2 gap-2 scrollbar-hide">
             {['All', ...Object.values(OrderStatus)].map((s: any) => (<button key={s} onClick={() => setFilter(s)} className={`px-4 py-1.5 rounded-full text-xs font-bold border transition-all whitespace-nowrap ${filter === s ? 'bg-slate-800 dark:bg-slate-100 text-white dark:text-slate-900 border-slate-800 dark:border-slate-100 shadow-md' : 'bg-white dark:bg-slate-900 text-slate-500 dark:text-slate-400 border-slate-200 dark:border-slate-800 hover:border-slate-300 dark:hover:border-slate-600 hover:bg-slate-50 dark:hover:bg-slate-800'}`}>{s === 'All' ? '全部订单' : OrderStatusCN[s as OrderStatus]}</button>))}
             {filter === 'delayed' && (<button onClick={() => setFilter('delayed')} className="px-4 py-1.5 rounded-full text-xs font-bold border transition-all whitespace-nowrap bg-red-600 text-white border-red-600 shadow-md flex items-center gap-1"><AlertTriangle size={12} />异常预警</button>)}
        </div>
      )}
      ...
      {selectedIds.size > 0 && (
          <div className="fixed bottom-6 left-1/2 transform -translate-x-1/2 z-50 bg-slate-900 text-white px-6 py-3 rounded-2xl shadow-2xl flex items-center gap-6 animate-fade-in border border-slate-700 w-[90%] md:w-auto">
              <div className="flex items-center gap-3 pr-4 border-r border-slate-700"><div className="w-6 h-6 rounded-full bg-indigo-500 flex items-center justify-center text-xs font-bold">{selectedIds.size}</div><span className="text-sm font-medium whitespace-nowrap">已选择</span></div>
              <div className="flex items-center gap-2 overflow-x-auto scrollbar-hide">
                  <button onClick={handleBatchExportAction} className="flex items-center gap-1.5 px-3 py-1.5 hover:bg-slate-800 rounded-lg text-xs font-medium transition-colors whitespace-nowrap"><Download size={14} /> 批量导出</button>
                  {!isTrash && (
                      <>
                        <div className="h-4 w-px bg-slate-700 mx-1"></div>
                        <div className="relative group"><button className="flex items-center gap-1.5 px-3 py-1.5 hover:bg-slate-800 rounded-lg text-xs font-medium transition-colors whitespace-nowrap"><CheckCircle2 size={14} /> 更改状态</button><div className="absolute bottom-full left-0 mb-2 w-32 bg-white dark:bg-slate-800 rounded-lg shadow-xl overflow-hidden hidden group-hover:block border border-slate-200 dark:border-slate-700 py-1">{Object.values(OrderStatus).map(status => (<button key={status} onClick={() => handleBatchStatusAction(status)} className="w-full text-left px-4 py-2 text-xs text-slate-700 dark:text-slate-300 hover:bg-indigo-50 dark:hover:bg-indigo-900/30 hover:text-indigo-600 block">{OrderStatusCN[status]}</button>))}</div></div>
                      </>
                  )}
                  <div className="h-4 w-px bg-slate-700 mx-1"></div>
                  <button onClick={handleBatchDeleteAction} className="flex items-center gap-1.5 px-3 py-1.5 hover:bg-red-900/50 text-red-400 hover:text-red-300 rounded-lg text-xs font-medium transition-colors whitespace-nowrap"><Trash2 size={14} /> {isTrash ? '彻底删除' : '移入回收站'}</button>
              </div>
              <button onClick={clearSelection} className="ml-2 p-1 hover:bg-slate-800 rounded-full text-slate-400 hover:text-white transition-colors"><X size={16} /></button>
          </div>
      )}
      ...
      {viewMode === 'board' && !isTrash ? (
          <div className="flex gap-4 overflow-x-auto pb-4 h-[calc(100vh-220px)] snap-x">
            {Object.values(OrderStatus).map(status => {
                ...
                return (<div key={status} className="flex-shrink-0 w-80 bg-slate-100 dark:bg-slate-900/50 rounded-xl flex flex-col snap-center border border-slate-200 dark:border-slate-800"><div className="p-3 border-b border-slate-200 dark:border-slate-800 flex justify-between items-center bg-slate-50 dark:bg-slate-800 rounded-t-xl sticky top-0 z-10"><h3 className="text-sm font-bold text-slate-700 dark:text-slate-200">{OrderStatusCN[status]}</h3><span className="bg-slate-200 dark:bg-slate-700 text-slate-600 dark:text-slate-300 px-2 py-0.5 rounded-full text-xs font-bold">{columnOrders.length}</span></div><div className="p-2 overflow-y-auto flex-1 custom-scrollbar space-y-2">{columnOrders.map(order => (<div key={order.id} onClick={() => onEdit(order)} className="bg-white dark:bg-slate-800 p-3 rounded-lg shadow-sm border border-slate-200 dark:border-slate-700 hover:shadow-md transition-all cursor-pointer group select-none relative"><div className="flex gap-3 mb-2"><div className="w-12 h-12 rounded bg-slate-50 dark:bg-slate-700 border border-slate-100 dark:border-slate-600 shrink-0 overflow-hidden">{order.imageUrl ? <img src={order.imageUrl} alt="" className="w-full h-full object-cover" /> : <div className="w-full h-full flex items-center justify-center text-slate-300 dark:text-slate-500"><Package size={16} /></div>}</div><div className="min-w-0"><h4 className="text-sm font-bold text-slate-800 dark:text-white line-clamp-2 leading-tight mb-1">{order.itemName}</h4><p className="text-xs text-slate-500 dark:text-slate-400">${order.priceUSD} x {order.quantity}</p></div></div><div className="flex items-center justify-between pt-2 border-t border-slate-50 dark:border-slate-700"><span className="text-[10px] bg-slate-100 dark:bg-slate-700 px-1.5 py-0.5 rounded text-slate-500 dark:text-slate-300">{order.platform}</span>{order.detailedStatus && <span className="text-[10px] text-indigo-500 dark:text-indigo-400 font-medium">{order.detailedStatus}</span>}</div></div>))}</div></div>);
            })}
          </div>
      ) : filteredOrders.length === 0 ? (
        <div className="text-center py-24 bg-white dark:bg-slate-900 rounded-xl border border-dashed border-slate-200 dark:border-slate-800"><div className="bg-slate-50 dark:bg-slate-800 w-16 h-16 rounded-full flex items-center justify-center mx-auto mb-4 border border-slate-100 dark:border-slate-700">{isTrash ? <Trash2 className="text-slate-300" size={32} /> : <Package className="text-slate-300" size={32} />}</div><h3 className="text-sm font-bold text-slate-800 dark:text-white">{isTrash ? '回收站为空' : '暂无相关订单'}</h3>{!isTrash && <p className="text-slate-500 text-xs mt-1">请尝试更换搜索关键词或筛选条件</p>}</div>
      ) : (
        <>
            {viewMode === 'grid' && !isTrash ? (
                <div className="grid grid-cols-1 xl:grid-cols-2 gap-6">
                    {filteredOrders.map(order => (
                        <div key={order.id} onClick={() => onEdit(order)} className={`bg-white dark:bg-slate-900 rounded-xl shadow-sm border transition-all duration-300 cursor-pointer overflow-hidden group relative ${selectedIds.has(order.id) ? 'ring-2 ring-indigo-500 border-indigo-500 bg-indigo-50/10 dark:bg-indigo-900/10' : 'border-slate-200 dark:border-slate-800 hover:shadow-lg hover:border-indigo-300 dark:hover:border-indigo-700'}`}><div onClick={(e) => toggleSelectOne(order.id, e)} className={`absolute top-3 right-3 z-10 w-6 h-6 rounded-md border flex items-center justify-center transition-all cursor-pointer ${selectedIds.has(order.id) ? 'bg-indigo-500 border-indigo-500 text-white' : 'bg-white/80 dark:bg-slate-800/80 border-slate-300 dark:border-slate-600 text-transparent hover:border-indigo-400 backdrop-blur-sm'}`}><Check size={14} strokeWidth={3} /></div><div className="p-5 flex flex-col sm:flex-row gap-6"><div className="w-full sm:w-48 h-48 sm:h-auto flex-shrink-0 bg-slate-50 dark:bg-slate-800 rounded-lg overflow-hidden border border-slate-100 dark:border-slate-700 flex items-center justify-center relative">{order.imageUrl ? <img src={order.imageUrl} alt={order.itemName} className="w-full h-full object-contain p-2" /> : <Package size={32} className="text-slate-300" />}<div className="absolute top-2 left-2">{getStatusBadge(order)}</div></div><div className="flex-1 min-w-0 flex flex-col justify-between"><div><h3 className="text-lg font-bold text-slate-900 dark:text-white truncate pr-4">{order.itemName}</h3><div className="text-xl font-bold text-slate-900 dark:text-white mt-2">${(order.priceUSD * order.quantity).toFixed(2)}</div></div><div className="mt-2 text-xs font-mono font-bold text-slate-400">{order.trackingNumber || ''}</div><div className="flex justify-end items-center gap-2 pt-4 border-t border-slate-100 dark:border-slate-800 mt-2"><button onClick={(e) => handleGenerateUpdate(order, e)} className="p-2 text-slate-400 hover:text-indigo-600"><MessageSquare size={16}/></button><button onClick={(e) => handleDelete(order.id, e)} className="p-2 text-slate-400 hover:text-red-600"><Trash2 size={16}/></button></div></div></div></div>
                    ))}
                </div>
            ) : (
                <div className="bg-white dark:bg-slate-900 rounded-xl shadow-sm border border-slate-200 dark:border-slate-800 overflow-hidden">
                    <div className="overflow-x-auto">
                        <table className="w-full text-sm text-left">
                            ...
                            <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
                                {filteredOrders.map(order => (
                                    <tr key={order.id} onClick={() => !isTrash && onEdit(order)} className={`cursor-pointer transition-colors group ${selectedIds.has(order.id) ? 'bg-indigo-50/50 dark:bg-indigo-900/20' : 'hover:bg-indigo-50/30 dark:hover:bg-slate-800'}`}>
                                        <td className="px-4 py-4 text-center align-top pt-6" onClick={(e) => e.stopPropagation()}><button onClick={(e) => toggleSelectOne(order.id, e)} className="flex items-center justify-center text-slate-300 dark:text-slate-600 hover:text-indigo-600">{selectedIds.has(order.id) ? <CheckSquare size={18} className="text-indigo-600"/> : <Square size={18} />}</button></td>
                                        <td className="px-4 py-4 align-top"><div className="flex gap-3"><div className="w-16 h-16 rounded bg-slate-100 dark:bg-slate-800 flex items-center justify-center shrink-0 border border-slate-200 dark:border-slate-700 overflow-hidden relative">{order.imageUrl ? <img src={order.imageUrl} alt={order.itemName} className="w-full h-full object-cover" /> : <Package size={20} className="text-slate-400" />}</div><div className="min-w-0 flex-1"><div className="font-bold text-slate-900 dark:text-white line-clamp-2 leading-tight mb-1">{order.itemName}</div><div className="font-medium text-slate-600 dark:text-slate-400 text-xs">${(order.priceUSD * order.quantity).toFixed(2)} <span className="text-slate-400 font-normal">(${order.priceUSD} x {order.quantity})</span></div>{order.clientOrderId && <div className="mt-1.5 flex items-center gap-1 text-[11px] text-indigo-600 bg-indigo-50 px-1.5 py-0.5 rounded w-fit"><Hash size={10} /><span className="font-mono font-bold">{order.clientOrderId}</span></div>}</div></div></td>
                                        <td className="px-4 py-4 align-top">
                                            <div className="space-y-3">
                                                <div className="flex items-center justify-between">{getStatusBadge(order)}{isTrash && <div className="text-[10px] font-mono text-red-500 bg-red-50 px-1.5 py-0.5 rounded">{getDaysLeft(order.deletedAt)}天后清除</div>}</div>
                                                {!isTrash && order.status !== OrderStatus.CANCELLED && <div className="w-full bg-slate-100 dark:bg-slate-800 h-1.5 rounded-full overflow-hidden flex"><div className={`h-full rounded-full transition-all duration-500 ${order.status === OrderStatus.DELIVERED ? 'bg-emerald-500' : order.status === OrderStatus.SHIPPED ? 'bg-indigo-500' : 'bg-blue-400'}`} style={{ width: `${getProgress(order.status)}%` }}></div></div>}
                                                <div className="space-y-2 pt-1">
                                                    {order.trackingNumber ? (
                                                        <div className="relative overflow-hidden bg-slate-50 dark:bg-slate-800/50 rounded-lg border border-slate-200 dark:border-slate-800 shadow-sm">
                                                             <div className="absolute left-0 top-0 bottom-0 w-1 bg-slate-300 dark:bg-slate-600"></div>
                                                             <div className="px-3 py-2 pl-4">
                                                                <div className="flex items-center justify-between mb-1"><span className="text-[10px] uppercase tracking-wider text-slate-400 font-bold flex items-center gap-1"><Hash size={10} /> TIKTOK平台单号</span></div>
                                                                <div className="font-mono text-xs text-slate-600 dark:text-slate-300 font-bold tracking-wide">{order.trackingNumber}</div>
                                                             </div>
                                                        </div>
                                                    ) : <div className="flex items-center gap-2 px-2 py-1.5 rounded-lg border border-dashed border-slate-200 dark:border-slate-800 text-slate-400 text-[10px]">待输入TIKTOK单号</div>}
                                                </div>
