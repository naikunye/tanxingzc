...
export const syncOrderLogistics = async (orders: Order[], token: string): Promise<{ updatedOrders: Order[], count: number, message: string }> => {
  ...
  if (!token) {
    ...
    return { 
      ...
      count: updatedCount, 
      message: updatedCount > 0 ? `已更新 ${updatedCount} 个订单状态` : '没有检测到可更新的状态'
    };
  }

  // API Mode: Only track supplierTrackingNumber (Tiktok platform ID is excluded as per requirement)
  try {
    ...
    if (data.code !== 0) throw new Error(`17TRACK Error: ${data.message}`);
    ...
    return { 
        updatedOrders: newOrders, 
        count: updatedCount, 
        message: `同步成功！已更新 ${updatedCount} 个订单的商家入库进度` 
    };
